...
void DoBelch(object oPC)
{
    ...
    if (nGender == GENDER_MALE)
        AssignCommand(oPC, PlaySound("as_pl_belchingm" + IntToString(d3())));
    else if (nGender == GENDER_FEMALE)
        DelayCommand(2.0, PlayVoiceChat(VOICE_CHAT_LAUGH, oPC));
}
...
void DoShriek(object oPC)
{
    ...
    int nGender = GetGender(oPC);
    if (nGender == GENDER_MALE)
        AssignCommand(oPC, PlaySound("as_pl_shriekm" + IntToString(d3())));
    else if (nGender == GENDER_FEMALE)
        AssignCommand(oPC, PlaySound("as_pl_shriekf" + IntToString(d3())));
}
